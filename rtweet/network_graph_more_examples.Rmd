...
foo %>% 
  rename(place = `dukmbb_location$location`) %>% 
  rename(coords = locality) %>% 
  filter(!is.na(place)) %>% 
  filter(is.null(coords))
...
str(locality)
str(locality, list.len = 60, max.level = 5)


locality["Wallace Wade Stadium"]

locality[48:52]

map(locality[48:52],4)
goo <- map_chr(locality[48:52],1)
class(goo)
goo[2]
...
x <- map(locality[48:52], `[`, 2:3)

y <- map_df(locality[48:52], `[`, 2:3)


sapply(dukmbb_location$location, function(x) lookup_coords(x, "country:US")) 
map(dukmbb_location$location, lookup_coords(location, "country:US"))


lookup_coords(dukmbb_location$location[1], "country:US")

foo <- map_df(dukmbb_location$location[1:10], lookup_coords(dukmbb_location$location))

map(dukmbb_location$location, lookup_coords(dukmbb_location$location))

lookup_coords("Elm City, NC")

dukmbb_location %>% 
  mutate(coords = lookup_coords(location))

dukmbb_location[1:25] %>% 
  filter(!is.na(location)) %>% 
  mutate(coords = as.list(geocode(dukmbb_location$location, output = c("latlon"), source = "dsk")))
  
as.list(geocode("Greenville, IL", output = c("latlon"), source = "dsk"))
...
geocode(dukmbb_location$location)
